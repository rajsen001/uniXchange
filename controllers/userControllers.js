...
const Email = require("../emailConfig/email");
...
const sendConfirmationEmail = asyncHandler(async (user, req) => {
  const confirmationToken = await user.createToken("emailConfirmation");
  await user.save({ validateBeforeSave: false });
  let confirmationURL = `${req.protocol}://${req.hostname}/confirmEmail/${confirmationToken}`;

  if (process.env.NODE_ENV == "development") {
    confirmationURL = `${req.protocol}://${req.hostname}:${process.env.FRONTEND_PORT}/confirmEmail/${confirmationToken}`;
    // confirmationURL = `${req.protocol}://${req.hostname}/confirmEmail/${confirmationToken}`;
  }

  console.log(confirmationURL, req.protocol, req.hostname);
